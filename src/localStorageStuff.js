...
const localStorageStuff = (function(){
  ...
  function saveToLocalStorage(){
    ...
    for(let i = 0; i < myTasks.myTasksList.length; i++){
      ...
      localStorage.setItem(`projectTask${i}`, myTasks.myTasksList[i].projectTask);
      localStorage.setItem(`priorityTask${i}`, myTasks.myTasksList[i].priorityTask);
      localStorage.setItem(`dateTask${i}`, myTasks.myTasksList[i].dateTask);
      localStorage.setItem(`contentTask${i}`, myTasks.myTasksList[i].contentTask);
    }

    for(let i = 0; i < myProjects.myProjectsList.length; i++){
      localStorage.setItem(`titleProject${i}`, myProjects.myProjectsList[i].titleProject);
      localStorage.setItem(`selectedProject${i}`, myProjects.myProjectsList[i].selected);
    }
  }
  ...
  function uploadFromLocalStorage(){
    ...
    for(let i = 0; i < lenTask; i++){
      ...
      let newTask = new Tasks(title, project, content, date, priority, taskSelected);
